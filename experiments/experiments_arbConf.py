...
for dataset_name in datasets:
    ...
    for c in confidence:
        ...
        outfile_rgttm = os.path.join(results_dir,
                                     "arbConf_" + str(c) + "performance_results_%s_%s_%s_gap%s_rgttm.csv" % (
        ...
        with open(outfile_rgttm, 'w') as fout2:
            ...
            fout2.write("%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (
                "dataset", "method", "cls", "nr_events", "n_iter", "metric", "score", "metric2", "score2", "metric3",
                "score3", "metric4", "score4", "metric5", "score5"))

            confidence = map(lambda x: c, preds_all)
            confidenceList = list(confidence)

            dt_results_rgttm = pd.DataFrame(
                {"actual": test_y_all, "predicted": preds_all, "nr_events": nr_events_all, "confidence": confidenceList,
                 "adjusted_preds": ""})
            ...
            for nr_events, group in dt_results_rgttm.groupby("nr_events"):
                ...
                mean = pd.DataFrame.mean(df_group_rgttm.actual)
                ...
                while index < len(df_group_rgttm.index):
                    df_group_rgttm.adjusted_preds[index] = adjust_with_regression_to_the_mean(
                        df_group_rgttm.predicted[index], mean, df_group_rgttm.confidence[index])
                ...
                if len(set(group.actual)) < 2:
                    fout2.write(
                        "%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s;%s\n" % (
                            dataset_name, method_name, cls_method, nr_events, -1, "auc", np.nan, "accuracy", np.nan,
                            "precision", np.nan, "recall", np.nan, "fscore", np.nan))
                else:
            ...
            dt_auc_collection = pd.DataFrame(
                {'nr_events': list_auc_collection_nr_events, 'actual': list_auc_collection_actual,
                 'predicted': list_auc_collection_preds, 'adjusted_preds': list_auc_collection_adjusted_preds,
                 "mean": list_auc_collection_mean, "confidence": list_auc_collection_confidence})
            comparison_path = os.path.join(results_dir,
                                           "arbConf_" + str(c) + "comparison_after_rgttm_%s_%s_%s_gap%s.csv" % (
                                               cls_method, dataset_name, method_name, gap))
        ...
        if gap > 1:
            outfile = os.path.join(results_dir,
                                   "arbConf_" + str(c) + "performance_results_%s_%s_%s_gap%s.csv" % (
                                       cls_method, dataset_name, method_name, gap))
        else:
            outfile = os.path.join(results_dir,
                                   "arbConf_" + str(c) + "performance_results_%s_%s_%s.csv" % (
